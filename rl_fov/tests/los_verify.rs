...
use rl_fov::{Fov, FovConfig, Los, VisionShape};
...
fn verify_los<L>(name: &str, mut los: L, map: &TestMap) where L: Los + FovConfig {
    let size = map.size();
