...
pub trait RlEcsPrivate<BINSTORE>
    ...
    fn purge_int(&mut self, id: IdInt) -> Result<(), Error>
        where BINSTORE: BinStoragePrivate;
...
impl<BINSTORE, const N: usize> RlEcs<BINSTORE, { N }> where BINSTORE: BinStorage + BinStoragePrivate, {
    ...
    pub fn contains(&mut self, id: Id) -> Result<bool, Error> { self.id.contains(id) }

    pub fn attach(&mut self, child_id: Id, parent_id: Id) -> Result<(), Error> { self.id.attach(child_id, parent_id) }

    pub fn detach(&mut self, child_id: Id) -> Result<(), Error> { self.id.detach(child_id) }
    ...
    pub fn get_parent_id(&mut self, child_id: Id) -> Result<Option<Id>, Error> { self.id.get_parent_id(child_id) }
    ...
    pub fn create_and_attach<ITEM>(&mut self, item: ITEM, parent_id: Id) -> Result<Id, Error>
        ...
        if self.get_child_id::<ITEM>(parent_id).unwrap_or(None).is_some() {
    ...
    where BINSTORE: BinStorage + BinStoragePrivate,
{
    fn get_child_id_int<ITEM>(&self, id: IdInt) -> Result<Option<Id>, Error>
        ...
        IdStoragePrivate::get_child_id(&self.id, id, idtype).map(|cido| cido.map(|cid| cid.to_id(child_gen)))
    }
    ...
    fn purge_int(&mut self, id: IdInt) -> Result<(), Error> {
        ...
        loop {
            ...
            let cid_opt =
                if let Some(mut iter) = IdStoragePrivate::child_iter_int(&self.id, id) { iter.next() } else { None };
...
mod tests {
    ...
    fn create() {
        ...
        let mut ecs = Builder::new().add_component::<TFlag>().add_component::<TMChildren>().finalize();
    ...
    fn attach() {
        ...
        let mut ecs = Builder::new().add_component::<TEntity>().add_component::<TMChildren>().finalize();

        // create te0 and check id
        assert_eq!(ecs.create(TEntity {}), tentity_id0);
        assert_eq!(ecs.create_and_attach(TMChildren {}, tentity_id0), Ok(tmchildren_id0));

        // check that all connections are valid
        assert_eq!(ecs.contains(tentity_id0), Ok(true));
        assert_eq!(ecs.get_child_id::<TMChildren>(tentity_id0), Ok(Some(tmchildren_id0)));
        assert_eq!(ecs.get::<TMChildren>(tmchildren_id0), Ok(Some(&TMChildren {})));
    ...
    fn detect_loop() {
        ...
        let mut ecs = Builder::new().add_component_with_type::<TUChildren1>(EntityRelationType::UniqueChildren)
    ...
    fn create_and_attach() {
        ...
        let mut ecs = Builder::new().add_component::<TEntity>()
                                    .add_component::<TFlag>()
        ...
        assert_eq!(ecs.create(TEntity {}), tentity_id0);
        assert_eq!(ecs.create_and_attach(TUChildren {}, tentity_id0).unwrap(), tuchildren_id0);
        assert_eq!(ecs.create_and_attach(TFlag {}, tuchildren_id0).unwrap(), tflag_id0);

        assert_eq!(ecs.contains(tentity_id0), Ok(true));
        assert_eq!(ecs.contains(tuchildren_id0), Ok(true));
        assert_eq!(ecs.contains(tflag_id0), Ok(true));
        assert_eq!(ecs.get::<TUChildren>(tuchildren_id0), Ok(Some(&TUChildren {})));
    ...
    fn remove_simple() {
        ...
        assert_eq!(*ecs.get::<TEntity>(ecs.id_iter::<TEntity>().next().unwrap()).unwrap().unwrap(), TEntity { i: 1, });
    }
    ...
    fn remove_advanced() {
        ...
        let mut ecs = Builder::new().add_component::<TEntity>().add_component::<TFlag>().finalize();
    ...
    fn purge() {
        ...
        // survive the purge
        let mut count = 0;
        <(Read<Target<TEntity>>, Read<Not<Child<TFlag>>>)>::build_query().run(&mut ecs, |te, tf| {
                                                                             assert_eq!(te.id, 1);
                                                                             assert_eq!(**tf, ());
                                                                             count += 1;
                                                                         });
    ...
    fn remove_tree_2() {
        ...
        assert_ne!(ecs.get_parent_id(child_of_parent).unwrap().unwrap(), parent_1);
        assert_eq!(ecs.get_parent_id(child_of_parent).unwrap().unwrap().to_id_internal_unchecked(),
                   parent_1.to_id_internal_unchecked());
        assert_eq!(ecs.get_parent_id(child_of_entity).unwrap().unwrap(), child_of_parent);
    }
}
